...
class GlCanvas : public QOpenGLWidget
{
  ...
  public:
    ...
    /**
     * Widget constructor. Initializes default parameters. This widget only
     * can have a DocumentView class-derived as parent.
     *
     ...
     */
    void screenToWorld(double x, double y, double z, double* wx, double* wy,
                       double* wz) const;
    /**
     * Map a point in screen coordinate system to world coordinate system
     * using current active camera settings for doing the mapping.
     ...
     * using current active camera settings for doing the mapping.
     */
    void worldToScreen(double wx, double wy, double wz, double* x, double* y,
                       double* z) const;
    /**
    ...
    void drawEllipse(const Point3& center, float axis1, float axis2,
                     float innerAxis1 = 0.0f, float innerAxis2 = 0.0f);

    /**
     * Draw an arc, a part of an ellipse with an optional hole in it. The
     * ellipse is defined by the major axis and the minor axis. The arc is
     ...
     *                   if not inner ellipse is desired.
     */
    void drawArc(const Point3& center, float startAngle, float endAngle,
                 float axis1, float axis2, float innerAxis1 = 0.0f,
  ...
  private:
    ...
    void drawEllipseWinCoord(const Point3& center, float startAngle,
                             float endAngle, float axis1, float axis2,
                             float innerAxis1, float innerAxis2);

    void drawSceneNode(Scene::shared_ptr& scene, SceneNode::shared_ptr& node);
