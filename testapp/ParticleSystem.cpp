...
int Impl::initialize_ocl(void)
{
    ...
    try
    {
        ...
        krnSubdivideEdges =
            cl::Kernel(program, "subdivide_edges_midpoint", &err);
        krnSubdivideFaces =
            cl::Kernel(program, "subdivide_faces_midpoint", &err);
        krnSubdivideAddFaces =
            cl::Kernel(program, "subdivide_add_faces", &err);
    }
    catch (cl::Error e)
    {
...
struct clParticleSystem Impl::create_from_surface(Subdivision* surface)
{
...
void Impl::update_surface_from_polygon(Subdivision*      surface,
                                       clParticleSystem& list)
{
    if (!surface)
    {
...
void Impl::step(Subdivision* s)
{
    ...
    try
    {
        ...
        cl::Buffer surfaceBuffer1(context, CL_MEM_READ_WRITE,
                                  sizeof(clParticleSystem), 0);

        unsigned long vertexBufferSize1 =
            surface1.p.capacity() * sizeof(clParticle);
        cl::Buffer vertexBuffer1(context,
                                 CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                                 vertexBufferSize1, surface1.p.data());

        clParticleSystem surface2 = create_from_surface(s);

        cl::Buffer surfaceBuffer2(context, CL_MEM_READ_WRITE,
                                  sizeof(clParticleSystem), 0);

        unsigned long vertexBufferSize2 =
            surface2.p.capacity() * sizeof(clParticle);
        cl::Buffer vertexBuffer2(context,
                                 CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                                 vertexBufferSize2, surface2.p.data());
        ...
        cl::Event event;
        queue.enqueueNDRangeKernel(krnSubdivideEdges, cl::NullRange,
                                   cl::NDRange(s->numEdges() / 2),
                                   cl::NullRange, &evtLst, &event);
...
static void step(ISurface* s, float time)
{
