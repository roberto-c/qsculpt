...
void getAllLightsRecursive(const std::shared_ptr<const SceneNode>& scene,
...
void getCameraRecursive(const std::shared_ptr<const SceneNode>& scene,
...
CameraNode::shared_ptr getCamera(const std::shared_ptr<const SceneNode>& doc)
{
...
void TestMaterial::setup(const std::shared_ptr<const SceneNode>& doc)
{
    ...
    GLint textureSamplerId =
        shaderProgram()->uniformLocation("textureSampler");
    ...
    if (textureSamplerId != -1)
        shaderProgram()->setUniform(textureSamplerId, 0);
...
float TestMaterial::exponent() const { return d->exponent; }

void TestMaterial::setExponent(float exp) { d->exponent = exp; }

Color TestMaterial::specular() const { return d->specular; }

void TestMaterial::setSpecular(const Color& c) { d->specular = c; }

Color TestMaterial::diffuse() const { return d->diffuse; }

void TestMaterial::setDiffuse(const Color& c) { d->diffuse = c; }

Color TestMaterial::ambient() const { return d->ambient; }

void TestMaterial::setAmbient(const Color& c) { d->ambient = c; }
