...
Surface_vertex(struct Surface * surface, uint vtxIid)
{
    return &surface->v.v[vtxIid-1];
}

inline global struct Edge *
Surface_edge(struct Surface * surface, uint edgeIid)
{
    return &surface->e.e[edgeIid - 1];
}

inline global struct Face *
Surface_face(struct Surface * surface, uint faceIid)
{
