...
class SubdivisionClosestPointTest
{
    ...
    void printSceneNode(Scene::shared_ptr  node,
                        const std::string& indent = "");

    void printSceneNode(SceneNode::shared_ptr node,
                        const std::string&    indent = "");
...
int SubdivisionClosestPointTest::setup()
{
    ...
    node2->transform().rotate(
        Eigen::AngleAxisf(M_PI_4, Vector3(0.f, 0.f, 1.f)));
...
int SubdivisionClosestPointTest::test()
{
    ...
    std::cerr << "Node2 Vertex IID: "
              << node2->surface()->getClosestPointAtPoint(p3);
    return 0;
}

int SubdivisionClosestPointTest::cleanup() { return 0; }
...
void SubdivisionClosestPointTest::printSceneNode(SceneNode::shared_ptr node,
                                                 const std::string&    indent)
{
    if (node)
        return;
...
void SubdivisionClosestPointTest::printSceneNode(Scene::shared_ptr  node,
                                                 const std::string& indent)
{
    if (node)
        return;
