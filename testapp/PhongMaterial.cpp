...
namespace
{
void getAllLightsRecursive(const SceneNode::const_shared_ptr&  scene,
...
void getCameraRecursive(const SceneNode::const_shared_ptr& scene,
...
CameraNode::shared_ptr getCamera(const SceneNode::const_shared_ptr& doc)
{
...
void PhongMaterial::setup(const std::shared_ptr<const SceneNode>& doc)
{
    ...
    if (doc)
    {
        ...
        if (mat)
            shaderProgram()->setUniform(mat,
                                        d->camera->camera()->modelView());
        mat = shaderProgram()->uniformLocation("glProjectionMatrix");
        if (mat)
            shaderProgram()->setUniform(mat,
                                        d->camera->camera()->projection());
    }
    ...
    if (diff != -1)
        shaderProgram()->setUniform(diff, d->diffuse.data());
    if (spec != -1)
        shaderProgram()->setUniform(spec, d->specular.data());
    if (amb != -1)
        shaderProgram()->setUniform(amb, d->ambient.data());
    if (exp != -1)
        shaderProgram()->setUniform(exp, d->exponent);
    if (camPosId != -1)
        shaderProgram()->setUniform(camPosId, camPos);
    if (lightPosId != -1)
        shaderProgram()->setUniform(lightPosId, lightPos);

    THROW_IF_GLERROR("Failed to set params");
}

float PhongMaterial::exponent() const { return d->exponent; }

void PhongMaterial::setExponent(float exp) { d->exponent = exp; }

Color PhongMaterial::specular() const { return d->specular; }

void PhongMaterial::setSpecular(const Color& c) { d->specular = c; }

Color PhongMaterial::diffuse() const { return d->diffuse; }

void PhongMaterial::setDiffuse(const Color& c) { d->diffuse = c; }

Color PhongMaterial::ambient() const { return d->ambient; }

void PhongMaterial::setAmbient(const Color& c) { d->ambient = c; }
