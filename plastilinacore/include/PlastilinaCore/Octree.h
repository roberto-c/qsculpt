...
namespace data
{
template <typename T, typename ToPointFn = data::ImplicitConvToPoint<T>>
class Octree;
...
class Octree : public data::ICollection<Type>
{
  ...
  public:
    ...
    inline bool append(const Type& v)
    {
        ...
        if (!m_rootNode->add(v))
        {
            std::cerr << "Octree::add : failed: Point: " + ::toString(v)
                      << std::endl;
            return false;
        }
        return true;
    ...
    inline void remove(const Type& v) { Q_UNUSED(v); }
    ...
    inline void reserve(int _size) { m_data.reserve(_size); }

    inline std::string toString() { return m_rootNode->toString(); }

    inline std::vector<Type> toStdVector() { return m_data; }

    inline const std::vector<Type> toStdVector() const { return m_data; }
