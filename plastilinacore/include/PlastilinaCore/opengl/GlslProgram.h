...
class DLLEXPORT GlslProgram
{
  public:
     ...
     * ACTIVE_ATTRIBUTES.
     * @param name string object to hold the name into. It is optional,
     * passing NULL in this parameter will not return the name of the
     * attribute.
     * @param type pointer to an enum to store the OpenGL type for the
     * attribute. This is optional, if NULL, then no attempt to get the type
     * is done.
     * @param size size for the type returned in units of the given type.
     */
    void activeAttrib(GLint index, std::string* name, GLenum* type,
                      GLint* size);
