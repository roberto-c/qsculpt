...
class DLLEXPORT ICollection
{
  public:
     ...
     * what distance it is.
     */
    virtual bool findClosest(T& out, const Point3& p,
                             float maxDistance = 0) const = 0;

    /**
     * Overload method @see findClosest
     */
    //        virtual bool findClosest(const T& out, const Point3& p, float
    //        maxDistance = 0) const = 0;
     ...
     */
    virtual bool findIntersect(const Ray&      ray,
                               ICollection<T>* bag = NULL) const = 0;

    /**
     *
     */
    virtual bool findIntersect(const AABB&     box,
                               ICollection<T>* bag = NULL) const = 0;
