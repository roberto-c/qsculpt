...
class OctreeNode
{
  ...
  private:
    ...
    std::vector<OctreeNode<D, ToPointFn>*> m_childrenNodes; // Children nodes.
...
template <typename D, typename ToPointFn>
OctreeNode<D, ToPointFn>::OctreeNode(OctreeNode<D, ToPointFn>* parent,
                                     int depth)
    : m_parent(parent)
    , m_hasChildren(false)
    , m_depth(depth)
...
bool OctreeNode<D, ToPointFn>::findClosest(const Point3& v, D* data)
{
    ...
    {
        bool res;
        for (auto it = m_childrenNodes.begin(); it != m_childrenNodes.end();
             ++it)
        {
...
template <typename D, typename ToPointFn>
std::string OctreeNode<D, ToPointFn>::toString()
{
    ...
    else
    {
        ...
        for (auto it = m_dataIndices.begin(); it != m_dataIndices.end(); ++it)
        {
            res.str("");
            res << indent;
            res << "point[" << it << "]:" << m_octree->toStdVector()[it];
        }
    }
    res << indent << "}" << std::endl;
    return res.str();
