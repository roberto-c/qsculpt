...
class Vector : public ICollection<Type>
{
  ...
  public:
    ...
    inline void add(const Type& e) { _data.push_back(e); }

    inline void remove(const Type& e) { _data.remove(_data.indexOf(e)); }
    ...
    inline Type* getData() { return _data.data(); }

    inline Type& operator[](int index) { return _data[index]; }

    inline const Type& operator[](int index) const { return _data[index]; }

    inline const Type& at(int index) const { return _data.at(index); }

    inline std::string toString() { return std::string(); }

    inline std::vector<Type> toStdVector() { return _data; }

    inline const std::vector<Type> toStdVector() const { return _data; }
  ...
  private:
    ...
    class TypeIterator : public IIterator<T>
    {
      ...
      protected:
         ...
         */
        TypeIterator(data::Vector<T>* col)
            : _col(col)
            , _index(0)
        {
        }
      ...
      public:
         ...
         *
         */
        IIterator<T>* clone() const { return new TypeIterator<T>(*this); }

        /**
         * Return true if the iterator has more elements (i.e. it is not at
         * the end)
         */
        bool hasNext() const
        {
        ...
        /**
         * Returns true if the iterator is not at the beginning of the
         * iteration
         */
         ...
         */
        T& next() { return _col->_data[_index++]; }

        /**
         * Returns the next element and advance the iterator by one.
         */
        const T& next() const { return _col->_data.at(_index++); }

        /**
         * Returns the previous elements and move the iterator one position
         * backwards.
         */
        T& previous() { return _col->_data[--_index]; }

        /**
         * Returns the previous elements and move the iterator one position
         * backwards.
         */
        const T& previous() const { return _col->_data.at(--_index); }
