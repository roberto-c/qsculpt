...
void WireFrameMaterial::setup(const std::shared_ptr<const SceneNode>& doc)
{
    ...
    if (outline != -1)
        shaderProgram()->setUniform(outline, d->outline.data());
    if (fill != -1)
        shaderProgram()->setUniform(fill, d->fill.data());

    THROW_IF_GLERROR("Failed to set params");
}

Color WireFrameMaterial::outlineColor() const { return d->outline; }

void WireFrameMaterial::setOutlineColor(const Color& c) { d->outline = c; }

Color WireFrameMaterial::fillColor() const { return d->fill; }

void WireFrameMaterial::setFillColor(const Color& c) { d->fill = c; }
