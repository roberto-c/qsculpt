...
namespace core {
    ...
    {
        std::vector<NSOpenGLPixelFormatAttribute> atrList;
            ...
            switch (name) {
                case CTX_ATR_GL_NSVIEW:
                    ...
                    if (!setAttribute(name, value) ) {
                        std::cerr << "failed to set attribute" << std::endl;
                    };
                    break;
        ...
        NSOpenGLPixelFormat * pixelFormat = [[NSOpenGLPixelFormat alloc] initWithAttributes:atrList.data()];
        glctx = [[NSOpenGLContext alloc] initWithFormat:pixelFormat shareContext:nil];
        [glctx setView:view];
    }
    ...
    bool Context::Impl::setAttribute(CtxAttribute name,
                                  CtxAttribute  value)
    {
        switch (name) {
            case CTX_ATR_GL_NSVIEW:
