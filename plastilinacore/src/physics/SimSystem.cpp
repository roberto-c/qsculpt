...
void SimSystem::step(float step)
{
    ...
    //        floor(Eigen::Vector3f(0,1,0).normalized(), floorPoint);
    ...
    for (auto a : d->actorList)
    {
        ...
        {
            Eigen::Vector3f planePoint = Eigen::Vector3f(
                0, -0.5f, 0); // plane.projection(Eigen::Vector3f(0,0,0));
            if (plane.normal().dot(a->x - planePoint) < 0)
            {
                // 2(N.L)N - L
                float           l = a->v.norm();
                Eigen::Vector3f r =
                    2 * (plane.normal().dot(a->v.normalized())) *
                        plane.normal() -
                    a->v.normalized();
                a->x = a->x0;
                a->v = -r * (l * 0.8f);
...
void SimSystem::addActor(Actor::shared_ptr& a) { d->actorList.push_back(a); }
...
void SimSystem::clearActors() { d->actorList.clear(); }

std::vector<Actor::shared_ptr>& SimSystem::actors() { return d->actorList; }
...
void SimSystem::clearForceFunctors() { d->forceList.clear(); }
...
void SimSystem::clearWalls() { d->wallList.clear(); }
}
