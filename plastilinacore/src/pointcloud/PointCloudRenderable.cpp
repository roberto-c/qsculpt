...
void PointCloudRenderable::renderObject(RenderState& state) const
{
    ...
    if (prog->programID() > 0)
    {
        ...
        if (matId != -1)
            prog->setUniform(matId, camera->modelView());
    ...
    if (matId != -1)
    {
        prog->setUniform(matId, node->transform() *
                                    node->parentTransform().matrix());
    }
...
void PointCloudRenderable::fillVertexBufferPoints(ISurface*     mesh,
