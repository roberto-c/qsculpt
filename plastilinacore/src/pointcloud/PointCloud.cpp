...
class PointCloud::FaceIterator : public IIterator<FaceHandle>
{
  ...
  public:
     ...
     *
     */
    IIterator<FaceHandle>* clone() const { return nullptr; };
     ...
     * Returns true if the iterator is not at the beginning of the iteration
     */
    bool hasPrevious() const { return false; }

    /**
     * Returns the next element and advance the iterator by one.
     */
    shared_ptr next() { return shared_ptr(nullptr); }

    /**
     * Returns the next element and advance the iterator by one.
     */
    const_shared_ptr next() const { return shared_ptr(nullptr); }

    /**
     * Returns the next element without advancing to the next
     */
    shared_ptr peekNext() { return shared_ptr(nullptr); }

    /**
     * Returns the next element without advancing to the next
     */
    const_shared_ptr peekNext() const { return shared_ptr(nullptr); }

    /**
     * Returns the previous elements and move the iterator one position
     * backwards.
     */
    shared_ptr previous() { return shared_ptr(nullptr); }

    /**
     * Returns the previous elements and move the iterator one position
     * backwards.
     */
    const_shared_ptr previous() const { return shared_ptr(nullptr); }
     ...
     * @param origin states the reference to jump.
     */
    bool seek(int pos, IteratorOrigin origin) const { return false; }
};
...
class PointCloud::EdgeIterator : public IIterator<EdgeHandle>
{
  ...
  public:
     ...
     *
     */
    IIterator<EdgeHandle>* clone() const { return nullptr; };
...
ISurface::size_t PointCloud::iid() const { return d->_iid; }

void PointCloud::setScene(Scene* scene) { d->scene = scene; }

Scene* PointCloud::scene() const { return d->scene; }

const IRenderable* PointCloud::renderable() const { return &d->renderable; }
...
void PointCloud::setColor(const Color& c) { d->color = c; }

Color PointCloud::color() const { return d->color; }

void PointCloud::setSelected(bool val) { d->isSelected = val; }

bool PointCloud::isSelected() const { return d->isSelected; }
...
void PointCloud::removeVertex(ISurface::size_t id)
{
    std::remove_if(d->vertices.begin(), d->vertices.end(),
                   [id](const Vertex* v) { return v->iid() == id; });
}

VertexHandle* PointCloud::vertex(ISurface::size_t iid)
{
    auto it =
        std::find_if(d->vertices.begin(), d->vertices.end(),
                     [iid](const Vertex* v) { return v->iid() == iid; });
...
ISurface::size_t PointCloud::addEdge(const EdgeHandle& edge) { return 0; }

ISurface::size_t PointCloud::addEdge(ISurface::size_t v1, ISurface::size_t v2)
{
    return 0;
}

ISurface::size_t PointCloud::numEdges() const { return 0; }

ISurface::size_t
PointCloud::addFace(const std::vector<ISurface::size_t>& vertexIndexes)
{
    return 0;
}

void PointCloud::replaceFace(
    ISurface::size_t                     index,
...
ISurface::size_t PointCloud::numFaces() const { return 0; }

FaceHandle* PointCloud::face(ISurface::size_t index) { return nullptr; }
...
std::vector<ISurface::size_t> PointCloud::getPointsInRadius(
    const Point3& p, float radius) const
{
    NOT_IMPLEMENTED
}

void PointCloud::adjustPointNormal(ISurface::size_t index) { NOT_IMPLEMENTED }

void PointCloud::lock() const { NOT_IMPLEMENTED }

void PointCloud::unlock() const { NOT_IMPLEMENTED }

bool PointCloud::hasChanged() { return d->hasChanged; }

void PointCloud::setChanged(bool val) { d->hasChanged = val; }
...
void PointCloud::setSelectedPoints(
    const std::vector<ISurface::size_t>& indicesArray){NOT_IMPLEMENTED}
...
Iterator<FaceHandle> PointCloud::faceIterator(){NOT_IMPLEMENTED}

Iterator<FaceHandle> PointCloud::constFaceIterator() const {NOT_IMPLEMENTED}

Iterator<EdgeHandle> PointCloud::edgeIterator(){NOT_IMPLEMENTED}
...
void PointCloud::meshChanged(ISurface* mesh){NOT_IMPLEMENTED}
...
PointCloud::VertexIterator::const_shared_ptr
PointCloud::VertexIterator::next() const
{
    Vertex* v = *_index;
    ++_index;
    return v;
...
PointCloud::VertexIterator::const_shared_ptr
PointCloud::VertexIterator::peekNext() const
{
    return *_index;
}
...
PointCloud::VertexIterator::const_shared_ptr
    PointCloud::VertexIterator::previous() const
{
    NOT_IMPLEMENTED
    //    --_index;
    //    return _index->second;
...
bool PointCloud::VertexIterator::seek(int pos, IteratorOrigin origin) const
{
    ...
    {
        std::cerr << "PointCloud::VertexIterator::seek: not supported"
                  << std::endl;
        return false;
