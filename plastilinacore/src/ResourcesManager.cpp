                                               ...
                                               const std::string subdir)
{
    ...
    CFStringRef cfstrType =
        CFStringCreateWithCString(NULL, type.c_str(), kCFStringEncodingUTF8);
    CFStringRef cfSubfir = CFStringCreateWithCString(NULL, subdir.c_str(),
    ...
    if (requestedBundle)
    {
        ...
        resourceURL = CFBundleCopyResourceURL(requestedBundle, resourceName,
                                              cfstrType, cfSubfir);
    }
    if (!resourceURL)
    {
        std::cerr << "Failed to find resourceURL. Trying framework bundle"
                  << std::endl;
        // Look for a bundle using its identifier
        requestedBundle = CFBundleGetBundleWithIdentifier(
            CFSTR("com.plastilinalabs.PlastilinaCore"));
        if (requestedBundle)
        {
            // Look for a resource in the main bundle by name and type.
            resourceURL = CFBundleCopyResourceURL(
                requestedBundle, resourceName, cfstrType, cfSubfir);
        }
    }
    if (resourceURL)
    {
        UInt8 aPath[1024] = {0};
        if (!CFURLGetFileSystemRepresentation(resourceURL, FALSE, aPath,
                                              sizeof(aPath)))
        {
