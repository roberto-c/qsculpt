...
SceneNode::SceneNodeIterator::SceneNodeIterator(
    const SceneNode::const_weak_ptr& parent)
    : _parent(parent)
    , _index(0)
{
}
...
bool SceneNode::SceneNodeIterator::hasPrevious() const { return false; }
...
SceneNode::SceneNodeIterator::const_shared_ptr
SceneNode::SceneNodeIterator::next() const
{
    // throw std::runtime_error("Not implemented");
    SceneNode::shared_ptr ptr = _parent->_d->children[_index++];
    return ptr;
...
SceneNode::SceneNodeIterator::shared_ptr
...
SceneNode::SceneNodeIterator::const_shared_ptr
SceneNode::SceneNodeIterator::previous() const
{
    throw std::runtime_error("Not implemented");
}
...
SceneNode::SceneNodeTreeIterator::SceneNodeTreeIterator(
    const SceneNode::const_weak_ptr& parent)
    : _parent(parent)
    , _nextIndex(0)
{
...
bool SceneNode::SceneNodeTreeIterator::hasNext() const
{
    ...
    //    bool res = _nextIndex >= 0 && ((_parent && _parent->count() >
    //    _nextIndex) || _levelStack.size() > 0);
    bool res =
        _nextIndex >= 0 && ((_parent && _parent->count() > _nextIndex));
    // TRACE(trace) << "NextIndex: " << _nextIndex << " ParentCount: " <<
    // ((_parent) ? _parent->count() : -1 ) << "\n";
    return res;
}

bool SceneNode::SceneNodeTreeIterator::hasPrevious() const { return false; }

SceneNode::SceneNodeTreeIterator::const_shared_ptr
SceneNode::SceneNodeTreeIterator::next() const
{
    // throw std::runtime_error("Not implemented");
    ...
    // TRACE(trace) << "RetType:" << ret->nodeType() << " NextIndex: " <<
    // _nextIndex << " ParentCount: " << ((_parent) ? _parent->count() : -1 )
    // << "\n";
    return ret;
}

SceneNode::SceneNodeTreeIterator::shared_ptr
SceneNode::SceneNodeTreeIterator::next()
{
    const SceneNode::SceneNodeTreeIterator& p =
        static_cast<const SceneNode::SceneNodeTreeIterator&>(*this);
    return std::const_pointer_cast<SceneNode>(p.next());
}

SceneNode::SceneNodeTreeIterator::shared_ptr
SceneNode::SceneNodeTreeIterator::previous()
{
    throw std::runtime_error("Not implemented");
}

SceneNode::SceneNodeTreeIterator::const_shared_ptr
SceneNode::SceneNodeTreeIterator::previous() const
{
    throw std::runtime_error("Not implemented");
}

bool SceneNode::SceneNodeTreeIterator::seek(int            pos,
...
bool SceneNode::itemIndex(const SceneNode::weak_ptr child,
                          size_t*                   index) const
{
    assert(_d);
    auto   c     = child.lock();
...
geometry::AABB SceneNode::boundingBox() const { return _d->bbox; }

geometry::AABB& SceneNode::boundingBox() { return _d->bbox; }
...
std::shared_ptr<Material> SurfaceNode::material() const { return material_; }
...
std::shared_ptr<Camera> CameraNode::camera() const { return d->camera; }

EffectNode::EffectNode(const std::shared_ptr<Material>& material,
                       const std::string&               name)
{
}
