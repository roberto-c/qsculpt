...
void FrameBufferObject::framebufferTexture(GLenum     attachment,
                                           Texture2D& texture, GLint level)
{
...
void FrameBufferObject::framebufferRenderbuffer(GLenum attachment,
                                                GLenum renderbuffertarget,
                                                RenderBuffer& renderbuffer)
{
    glFramebufferRenderbuffer(GL_RENDERBUFFER, attachment, renderbuffertarget,
                              renderbuffer.oid());
    THROW_IF_GLERROR("FrameBufferObject::framebufferRenderbuffer: failed to "
                     "attach RenderBuffer");
}
                                      ...
                                      GLsizei width, GLsizei height)
{
    glRenderbufferStorageMultisample(GL_RENDERBUFFER, samples, internalformat,
                                     width, height);
    THROW_IF_GLERROR(
        "RenderBuffer::storageMultisample: failed to set storage");
}
