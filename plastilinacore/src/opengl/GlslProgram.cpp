...
GlslProgram::GlslProgram(GLuint pid)
    : d(new Impl(0))
{
    if (glIsProgram(pid))
    {
...
GlslProgram::~GlslProgram() { glDeleteProgram(d->progId_); }

GLuint GlslProgram::programID() const { return d->progId_; }

void GlslProgram::setProgramID(GLuint pid) { d->progId_ = pid; }
...
bool GlslProgram::GlslProgram::link()
{
    ...
    glGetProgramiv(d->progId_, GL_ACTIVE_ATTRIBUTE_MAX_LENGTH,
                   &d->maxNameSize);
...
void GlslProgram::activeAttrib(GLint index, std::string* name, GLenum* type,
                               GLint* size)
{
    assert(d->progId_);
    if (!name && !type)
    ...
    glGetActiveAttrib(d->progId_, index, bufSize, NULL, size, type,
                      buffer.data());
    THROW_IF_GLERROR(__func__);
    if (name)
    {
...
void GlslProgram::bindFragDataLocation(GLuint             colorNumber,
