...
namespace vulkan
{
    ...
    struct Context::Impl {
        ...
        uint32_t findQueue(const vk::PhysicalDevice& physicalDevice, const vk::QueueFlags& flags, const vk::SurfaceKHR& presentSurface = vk::SurfaceKHR()) const {
    ...
    Context::Context(VkCtxFlags flags)
        : d(new Impl(flags))
    {
        bool enableValidation = static_cast<uint32_t>(flags) & static_cast<uint32_t>(VkCtxFlags::EnableValidation);
        {
            // Vulkan instance
        ...
        // Vulkan device
        {
            ...
            uint32_t graphicsQueueIndex = d->findQueue(physicalDevice, vk::QueueFlagBits::eGraphics);
        ...
        if (enableValidation) {
            //debug::setupDebugging(d->instance, vk::DebugReportFlagBitsEXT::eError | vk::DebugReportFlagBitsEXT::eWarning);
        }

        //if (enableDebugMarkers) {
        //    debug::marker::setup(device);
        //}
        d->pipelineCache = d->device.createPipelineCache(vk::PipelineCacheCreateInfo());
        // Find a queue that supports graphics operations
        d->graphicsQueueIndex = d->findQueue(physicalDevice, vk::QueueFlagBits::eGraphics);
