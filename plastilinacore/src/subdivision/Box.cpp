...
void Box::initPoints()
{
    ...
    vertex(vertexID[0])->cast<const Vertex*>()->normal() =
        Vector3(-hw, hh, -hd).normalized();
    vertex(vertexID[1])->cast<const Vertex*>()->normal() =
        Vector3(hw, hh, -hd).normalized();
    vertex(vertexID[2])->cast<const Vertex*>()->normal() =
        Vector3(hw, -hh, -hd).normalized();
    vertex(vertexID[3])->cast<const Vertex*>()->normal() =
        Vector3(-hw, -hh, -hd).normalized();
    ...
    vertex(vertexID[4])->cast<const Vertex*>()->normal() =
        Vector3(-hw, hh, hd).normalized();
    vertex(vertexID[5])->cast<const Vertex*>()->normal() =
        Vector3(hw, hh, hd).normalized();
    vertex(vertexID[6])->cast<const Vertex*>()->normal() =
        Vector3(hw, -hh, hd).normalized();
    vertex(vertexID[7])->cast<const Vertex*>()->normal() =
        Vector3(-hw, -hh, hd).normalized();
...
#define CELL(x, y) (((y) * (m + 1)) + (x))
    ...
    for (i = 0; i <= m; ++i)
    {
        ...
        for (j = 0; j <= n; ++j)
        {
            ...
            static_cast<Vertex*>(vertex(vertexID[0]))->texcoords() =
                Point2(u, v);
            numVtx++;
            x += wstep;
            u += 1 / m;
