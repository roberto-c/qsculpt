...
class FaceVertexIterator : public IIterator<VertexHandle>
{
  ...
  public:
    ...
    virtual bool operator==(const IIterator& rhs)
    {
        if (this == &rhs)
            return true;
        const FaceVertexIterator* r =
            static_cast<const FaceVertexIterator*>(&rhs);
        return this->surface == r->surface && this->face == r->face &&
               this->currentEdge == r->currentEdge;
    }

    friend bool operator==(const FaceVertexIterator& lhs,
                           const FaceVertexIterator& rhs)
    {
        return lhs.surface == rhs.surface && lhs.face == rhs.face &&
...
Iterator<VertexHandle> Face::vertexIterator(const ISurface* surface)
{
    const GpuSubdivision* gpusurf =
        dynamic_cast<const GpuSubdivision*>(surface);
...
Iterator<EdgeHandle> Face::edgeIterator(ISurface* surface){NOT_IMPLEMENTED}

Iterator<EdgeHandle> Face::constEdgeIterator(ISurface* surface) const {
    NOT_IMPLEMENTED}
...
{
    _dataBuffer = std::unique_ptr<VertexBuffer>(
        BOManager::getInstance()->createVBO("GPUSUBDIVISION", surface));
}
...
class GpuSubdivision::VertexIterator : public IIterator<VertexHandle>
{
  ...
  public:
    ...
    virtual bool operator==(const IIterator& rhs)
    {
        ...
        return this->_surface == r->_surface && this->_level == r->_level &&
               this->_index == r->_index;
    }

    friend bool operator==(const VertexIterator& lhs,
                           const VertexIterator& rhs)
    {
        return lhs._surface == rhs._surface && lhs._level == rhs._level &&
...
ISurface::size_t GpuSubdivision::iid() const { return _d->_iid; }
...
void GpuSubdivision::initPoints()
{
    ...
    _d->_dataBuffer->setBufferData(buffer.data(),
                                   buffer.size() * sizeof(Vector4f));

    unlock();
}

bool GpuSubdivision::hasChanged() { return _d->_hasChanged; }
...
void GpuSubdivision::setSelectedPoints(
    const std::vector<VertexHandle::size_t>& indicesArray)
{
    //_d->_selectedPoints = indicesArray;
}

void GpuSubdivision::setScene(Scene* scene) { _d->_scene = scene; }
...
void GpuSubdivision::setColor(const Color& color)
{
    ...
    while (it.hasNext())
    {
        static_cast<Vertex*>(it.next())->c =
            utils::convert_to<cl_float4>(color.data());
    }
}
...
void GpuSubdivision::setSelected(bool val) { _d->_selected = val; }

bool GpuSubdivision::isSelected() const { return _d->_selected; }
...
EdgeHandle::size_t GpuSubdivision::addEdge(VertexHandle::size_t v1,
                                           VertexHandle::size_t v2)
{
    assert(_d != nullptr && _d->_lock.owns_lock());
    assert(_d && _d->_vertices && v1 <= _d->_vertices->size() &&
           v2 <= _d->_vertices->size());
...
EdgeHandle::size_t GpuSubdivision::addEdge(VertexHandle* tail,
                                           VertexHandle* head)
{
    assert(_d != nullptr && _d->_lock.owns_lock());
    assert(tail && head);
...
EdgeHandle::size_t GpuSubdivision::edge(VertexHandle::size_t iidVtxTail,
...
FaceHandle::size_t GpuSubdivision::addFace(
    const std::vector<VertexHandle::size_t>& vertexIndexList)
{
    ...
    {
        Edge::size_t iid =
            edge(vertexIndexList[i], vertexIndexList[(i + 1) % size]);
        if (iid == 0)
            iid =
...
void GpuSubdivision::replaceFace(
...
FaceHandle::size_t
GpuSubdivision::getFaceIndexAtPoint(const Point3& /*p*/) const
{
    NOT_IMPLEMENTED
    return -1;
}
...
VertexHandle::size_t
GpuSubdivision::getClosestPointAtPoint(const Point3& p) const
{
...
GpuSubdivision::getPointsInRadius(const Point3& /*p*/, float /*radius*/) const
{
...
void GpuSubdivision::adjustPointNormal(size_t /*index*/){NOT_IMPLEMENTED}
...
GpuSubdivision::VertexIterator::VertexIterator(const GpuSubdivision* surface,
                                               int                   level)
    : _surface(surface)
    , _level(level)
{
...
bool GpuSubdivision::VertexIterator::seek(int            pos,
                                          IteratorOrigin origin) const
{
    switch (origin)
    {
    case Iter_Current:
    ...
    {
        std::cerr << "GpuSubdivision::VertexIterator::seek: not supported"
                  << std::endl;
        NOT_IMPLEMENTED
        return false;
    }
...
GpuSubdivision::FaceIterator::FaceIterator(const GpuSubdivision* surface,
                                           int                   level)
    : _surface(surface)
    , _level(level)
{
...
void GpuSubdivision::Impl::render(RenderState& state) const
{
...
void GpuSubdivision::Impl::copyToDevice() {}
...
IIterator<VertexHandle>* FaceVertexIterator::clone() const
{
...
bool FaceVertexIterator::hasNext() const
{
    assert(surface && face && "surface or face are null");
    TRACE(debug) << "hasNext: Face->edge=" << face->edge
                 << " currentEdge->next =" << currentEdge->next;
    return currentEdge->iid() != Edge::EDGE_END.iid();
}

bool FaceVertexIterator::hasPrevious() const {NOT_IMPLEMENTED}

VertexHandle::shared_ptr FaceVertexIterator::next()
{
    assert(surface && face && "surface or face are null");
    assert(currentEdge->iid() && "currentEdge is the end");
    currentEdge = &surface->edge(currentEdge->next)
                       ->cast<const core::gpusubdivision::Edge>();
    TRACE(debug) << "    next:" << currentEdge->iid();
    return const_cast<const VertexHandle::shared_ptr>(
        surface->vertex(currentEdge->head));
    ;
}

VertexHandle::const_shared_ptr FaceVertexIterator::next() const
{
    assert(surface && face && "surface or face are null");
    currentEdge = &surface->edge(currentEdge->next)
                       ->cast<const core::gpusubdivision::Edge>();
...
VertexHandle::shared_ptr FaceVertexIterator::peekNext()
{
    assert(surface && face && "surface or face are null");
    TRACE(debug) << "    current:" << currentEdge->iid();
    return const_cast<VertexHandle::shared_ptr>(
        surface->vertex(currentEdge->head));
}

VertexHandle::const_shared_ptr FaceVertexIterator::peekNext() const
{
    assert(surface && face && "surface or face are null");
    TRACE(debug) << "    current:" << currentEdge->iid();
    return const_cast<VertexHandle::shared_ptr>(
...
VertexHandle::shared_ptr FaceVertexIterator::previous(){NOT_IMPLEMENTED}

VertexHandle::const_shared_ptr FaceVertexIterator::previous() const
{
    NOT_IMPLEMENTED
}

bool FaceVertexIterator::seek(int pos, IteratorOrigin origin) const
{
    switch (origin)
    {
    case Iter_Current:
    ...
    if (pos > 0)
    {
        ...
        {
            currentEdge =
                &(surface->edge(currentEdge->next)->cast<const Edge>());
            ++pos;
        }
    }
    else if (pos == 0)
